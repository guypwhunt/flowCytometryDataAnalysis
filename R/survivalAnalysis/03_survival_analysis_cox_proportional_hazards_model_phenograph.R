...
combinedDf[is.na(combinedDf$outcomeDeathDate), "survivalFromVisit"] <-
  dateOfLateOutCome

combinedDf$fileName <- factor(combinedDf$fileName)

combinedDf$survivalFromVisit <-
  combinedDf$survivalFromVisit * 365.25
...
# Censored Analysis
minDF <-
  combinedDf[, c("survivalFromVisit",
                 "status",
                 clinicalAndBiologicalCovariates)]
minDF <- na.omit(minDF)
...
univ_formulas <- sapply(clinicalAndBiologicalCovariates,
                        function(x)
                          as.formula(paste('Surv(survivalFromVisit, status)~', x)))
...
saveRDS(censored.clinical.res.cox, file = paste0(
  ...
  clusterName,
  "SurvivalFromVisitClinicalModel.rds"
))
...
write.csv(
  ...
  paste0(
    "data/survivalAnalysis/",
    clusterName,
    "SurvivalFromVisitClinicalModel.csv"
  ),
  row.names = TRUE
)
...
censored.clinical.biological.res.cox <-
  coxph(
    Surv(survivalFromVisit, status) ~ ageAtVisit + sex + ethnicityID +
      onset + timeFromOnsetToVisitInYears + riluzole + delataAlsfrsRScore +
      GPR32_median_Positive_Naïve_B_Cells + GPR32_median_Positive_HLA_Negative_DR_Negative_Or_Low_Classical_Monocytes +
...
saveRDS(censored.clinical.biological.res.cox, file = paste0(
  "data/survivalAnalysis/",
  clusterName,
  "SurvivalFromVisitBiologicalModel.rds"
))
write.csv(
  ...
  paste0(
    "data/survivalAnalysis/",
    clusterName,
    "SurvivalFromVisitBiologicalModel.csv"
  ),
  row.names = TRUE
)
...
write.csv(
  ...
  paste0(
    "data/survivalAnalysis/",
    clusterName,
    "SurvivalFromVisitClinicalVsBiologicalModel.csv"
  ),
  row.names = TRUE
)
print(x)
