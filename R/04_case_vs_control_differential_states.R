...
'fastVsSlowVisitOneOneClusterDifferentialStatesStatistics.csv
fastVsSlowVisitOneOneClusterDifferentialAbundanceStatistics.csv
fastVsSlowVisitOneDifferentialStatesStatistics.csv
fastVsSlowVisitOneDifferentialAbundanceStatistics.csv'

'caseVsControlVisitOneOneClusterDifferentialStatesStatistics.csv
caseVsControlVisitOneOneClusterDifferentialAbundanceStatistics.csv
caseVsControlVisitOneDifferentialStatesStatistics.csv
caseVsControlVisitOneDifferentialAbundanceStatistics.csv'
...
allBCellsDS <-
  read.csv(
    "data/bCells/differentialTestingOutputs/caseVsControlVisitOneOneClusterDifferentialStatesStatistics.csv"
  )
allBCellsDS[, "panel"] <-
  "bCells"
...
bCellsClustersDS <-
  read.csv(
    "data/bCells/differentialTestingOutputs/caseVsControlVisitOneDifferentialStatesStatistics.csv"
  )
...
allMonocytesDS <-
  read.csv(
    "data/monocytes/differentialTestingOutputs/caseVsControlVisitOneOneClusterDifferentialStatesStatistics.csv"
  )
...
monocytesClustersDS <-
  read.csv(
    "data/monocytes/differentialTestingOutputs/caseVsControlVisitOneDifferentialStatesStatistics.csv"
  )
...
allTCellsDS <-
  read.csv(
    "data/tCells/differentialTestingOutputs/caseVsControlVisitOneOneClusterDifferentialStatesStatistics.csv"
  )
allTCellsDS[, "panel"] <-
  "tCells"
...
tCellsClustersDS <-
  read.csv(
    "data/tCells/differentialTestingOutputs/caseVsControlVisitOneDifferentialStatesStatistics.csv"
  )
...
allSenescenceDS <-
  read.csv(
    "data/senescence/differentialTestingOutputs/caseVsControlVisitOneOneClusterDifferentialStatesStatistics.csv"
  )
...
senescenceClustersDS <-
  read.csv(
    "data/senescence/differentialTestingOutputs/caseVsControlVisitOneDifferentialStatesStatistics.csv"
  )
senescenceClustersDS[, "panel"] <-
  "senescence"
senescenceClustersDS[, "typeOfCells"] <- c(senescentClusterNames,senescentClusterNames)
...
df[df[, "marker_id"]=="GPR32...AF488.A","marker_id"] <- "GPR32"
df[df[, "marker_id"]=="GPR32.AF488.A","marker_id"] <- "GPR32"
df[df[, "marker_id"]=="FPRL1...AF647.A","marker_id"] <- "FPRL1"
df[df[, "marker_id"]=="FPRL1.AF647.A","marker_id"] <- "FPRL1"
...
# GPR32 Plot
par(mar = c(1, 1, 1, 1))
p <- ggplot(data = df[df[, "marker_id"] == "GPR32",],
            aes(
              x = logFC,
  ...
  geom_text_repel() +
  ggtitle("GPR32 Differential Expression") +
  xlab("Log Fold Change") + ylab("0 - Log Adjusted P-Value")
print(p)

# FPRL1 Plot
par(mar = c(1, 1, 1, 1))
p <- ggplot(data = df[df[, "marker_id"] == "FPRL1",],
            aes(
              x = logFC,
              y = minus_log_fdr_adjusted_p_val,
              col = fdr_diff_expressed,
  ...
  scale_colour_manual(values = mycolors) +
  geom_text_repel() +
  ggtitle("FPRL1 Differential Expression") +
  xlab("Log Fold Change") + ylab("0 - Log Adjusted P-Value")
print(p)

df[df[,"fdr_adjusted_p_val"] < 0.05 & !is.na(df[,"fdr_adjusted_p_val"]),
   c("typeOfCells", "panel","cluster_id", "marker_id", "logFC","fdr_adjusted_p_val")]
