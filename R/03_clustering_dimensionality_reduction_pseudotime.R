...
label_HSNE_dm <- df%>%group_by(fileName)%>%select(DC1, DC2)%>%summarize_all(mean)
...
gc()
jpeg(file = paste0(figureDirectory,"diffusionmap1.jpeg"))
ggplot(df, aes(x=DC1, y=DC2, color=fileName))+geom_point(size=0.1)+theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor=element_blank())+geom_label_repel(aes(label=fileName), data=label_HSNE_dm)+guides(colour=FALSE)
dev.off()
gc()
jpeg(file = paste0(figureDirectory,"diffusionmap2.jpeg"))
...
dev.off()
gc()
jpeg(file = paste0(figureDirectory,"diffusionmap3.jpeg"))
...
######## FREQUENCIES ########
#############################
# As an example, we show the cluster frequencies of the HSNE clusters. To check the other cluster frequencies, adjust df$fileName
# check frequency of each cluster per sample
#counts <- as.data.frame.matrix(table(df$sampleID,df$fileName))
counts <- as.data.frame.matrix(table(rownames(df),df$fileName))
...
jpeg(file = paste0(figureDirectory,"count1.jpeg"))
...
dev.off()
jpeg(file = paste0(figureDirectory,"count2.jpeg"))
...
df$merged_HSNE <-gsub("CD4-1$", "Naive", df$fileName)
